...
int is_flow_finished(struct nat_connection *conn)
{
	...
	if(conn->internal_fin==1 || conn->external_fin==1) return 0;   
	else return 1;
}
...
struct nat_mapping *nat_insert_mapping(struct list_head *mapping_list, u32 internal_ip, u16 internal_port)
{
	...
	mapping_entry->external_port = htons(assign_external_port()); 					//在nat表中全部按照网络字节序存储
...
static int get_packet_direction(char *packet)
{
	...
	if(daddr == external_ip) return DIR_IN;          					
	else return DIR_OUT;
