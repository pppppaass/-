...
// the IP address of the iface, send ICMP echo reply; otherwise, hand it over to
// tcp stack.
void handle_ip_packet(iface_info_t *iface, char *packet, int len)
{
	...
	if (daddr == iface->ip) {
		...
		log(ERROR, "received packet with destination IP not belonging to the iface.");
	}
...
// run application, either server or client, according to the arguments
